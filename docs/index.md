...
- `Geb/Mathlib/CategoryTheory/FreeCoprodCompDisc.lean` — the free
  ...
  order). `coprodPair`/`plus` are the binary coproduct and its
  ...
  isomorphisms of the summands. No mathlib
...
- `Geb/Mathlib/Data/PFunctor/IndRec/` — codes for positive
  ...
  (the `IR I I` case) are deferred (see `TODO.md` § Complete
  Theorem 2.4 for `IndRec`). `univCode` instantiates the theory: the
  code of the universe generated by an arbitrary family of starting
  ...
  `IR.precomp` precomposes a code along a coproduct (the `γ^i` of
  ...
  deviation to a pointwise isomorphism. `IR.interpDeltaIso`
