...
`coequalizerCocone`, cited as `[nLabCoequalizer]` in the module
docstrings of both `Quotient.lean` and `Coequalizer.lean`, the
statement being the same one in each.

The key is recorded here, not deferred: `CONTRIBUTING.md` § Cite the
literature requires the identifier in the spec as well as the plan and
the Lean source, and `AGENTS.md` § Verify agent claims requires it
verified against the primary source before it is recorded. The entry
is verified by reading it. Its Examples section states "For 𝒞 = Set,
the coequalizer of two functions f, g is the quotient set by the
equivalence relation generated by the relation f(x) ∼ g(x) for all
x ∈ X", and its Definition section gives the universal property in the
form `π_desc` and `desc_uniq` take.

An nLab entry cited as itself is verifiable; a textbook locator is not,
absent the book. That is why the source here is the entry rather than
Mac Lane and Moerdijk's _Sheaves in Geometry and Logic_, whose key
`docs/references.bib` already carries but which nothing in the
repository cites, and whose section for colimits in `Set` this
workstream cannot check. The group's standing obligation draws the same
line for the skeleton locator, where the nLab entry attests Mac Lane
and Riehl and the obligation records that attestation is not
verification.
...
namespace because `size_union` and `size_push` are not about `Sized`,
so one `Batteries.UnionFind` block covers the module where a nested
`Sized` block would have to be opened and closed around them.
...
"currently" list, one of W4's `TODO.md` edits. It does not touch the
item's scoping criterion, which reads
...
`UnionFind.Sized.apply_root_ofEdges` throughout, in prose and in code
blocks alike. Likewise W1's constructors are `Hom.ofVec` and
`Hom.toVec_ofVec`, not bare `ofVec`: W1 declares them in
`FinSetSkel.Hom`, and its own test parallel writes them qualified.
...
`Fin.compressEquiv` per class. `π Y v` is `Hom.ofVec (Vector.ofFnC …)`
...
`UnionFind.Sized.root_root`; it binds the equivalence in a `let` likewise.
`desc Y v h` is `Hom.ofVec (Vector.ofFnC fun c ↦ h.toVec.get (r.get c))`
...
`comp_π` instantiates `UnionFind.Sized.root_ofEdges_eq_of_mem` at the
membership witness supplied by `List.mem_map` and `List.mem_finRange`. `edges`
...
instantiates `UnionFind.Sized.apply_root_ofEdges` at `h.toVec.get`,
whose hypothesis is `w` read indexwise through W1's `comp_get`, and
consumes
...
9". Each family binds W3 through W5, and two name W4. W4 answers all
three, and the amended closing paragraph as well: that paragraph now
adds that where the only instance in scope is choice-dependent a
choice-free module supplies its own, and no class W4 uses is in that
position — every instance W4 resolves is axiom-free at the arguments it
is resolved at.
...
`hasCoequalizers_of_hasColimit_parallelPair` requires of its instance
argument, which quantifies `{X Y} {f g}` implicitly.
...
- `Data/UnionFind/OfEdges.lean` — a fold over a small edge list. What
  ...
  the same ground as the second bullet below.
  `UnionFind.Sized.root_ofEdges_eq_of_mem` and
  `UnionFind.Sized.apply_root_ofEdges` are instantiated at that list as
...
- A module docstring for each of the three new source modules, with the
  ...
  `Quotient.lean`'s and `Coequalizer.lean`'s carry `[nLabCoequalizer]`
  under `## References`.
- A `/-- … -/` docstring on every `def`, `instance` and theorem of the
  three modules, per `docs/rules/lean-coding.md` § Comment and
  docstring rules, which makes them mandatory for every `def` and
  `instance` and for every theorem of public interest — which here is
  every theorem, each being part of the stated interface. The
  wrapper's unnamed `HasCoequalizers` instance is included.
...
the same footing. W4 also appends to
`GebMeta.classicalAllowedModules`, to `docs/index.md`, to the
`FinSetSkel` index files, and to `Geb/Mathlib/Data.lean` and
`GebTests/Mathlib/Data.lean`. W3 appends to several of the same, and
branch
...
whichever of them merges first. The obligation's fourth item, "any
shared directory index file", covers the `FinSetSkel` index files and
the two `Data.lean` index files alike.

W4's dependency on `feat/choice-free-primitives` is its `TODO.md` text
alone. That branch is specified to carry `Equiv.arrowCongrLeftC` and the
`LawfulBEq (Fin n)` instance its equality-API paragraph describes,
which that text leaves unnamed; § Constraint 9 above records that W4
reaches
