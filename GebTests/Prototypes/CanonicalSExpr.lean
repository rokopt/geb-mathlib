...
public import Geb.Prototypes.CanonicalSExpr  -- shake: keep; #guard needs it
public meta import Geb.Prototypes.CanonicalSExpr  -- shake: keep; #guard needs it
...
tree is `sampleAst` from `GebTests.Prototypes.ConcreteSyntax`, so the
...
`GebTests.Prototypes.ConcreteSyntax` asserts. -/
...
`GebTests.Prototypes.ConcreteSyntax` records the same three for
