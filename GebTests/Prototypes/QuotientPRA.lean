...
the signature with a nullary {lit}`leaf` and a binary {lit}`node` and the one-step
equation {lit}`node x y = node y x`. The tests compute the endpoints of the
...
/-- Commutativity of the node: {lit}`node x y = node y x`, with the variables
{lit}`false ↦ x`, {lit}`true ↦ y`. -/
