...
public import Geb.Prototypes.Definition -- shake: keep

public meta import Geb.Prototypes.BitStream -- shake: keep
public meta import Geb.Prototypes.Definition -- shake: keep

set_option doc.verso true in
/-!
# Examples of definitions and structural references

An export tree supplies typed positions to an equation block. Linking, derived operations,
...
* {lit}`layout` supplies two export positions.
...
definition, substitution, structural position, bitstream, test
-/
...
/-- The first structural position. -/
def first : Position layout := ⟨0, ()⟩

/-- The second structural position. -/
def second : Position layout := ⟨1, ()⟩
...
def nestedPosition : Position nestedLayout := ⟨1, ⟨0, ()⟩⟩
...
def equations : Block arithmetic Unit (Position layout) := fun i ↦
  Fin.cases (.pure (.inl ()))
...
def values (i : Position layout) : ℕ := Fin.cases 7 (fun _ ↦ 14) i.1
...
#guard nestedPosition.1.val = 1
#guard nestedPosition.2.1.val = 0
