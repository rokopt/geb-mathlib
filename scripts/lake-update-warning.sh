...
if [ "$allowed" -eq 0 ]; then
  ...
  # if it is renamed in a future jj, adjust this script and
  # `pre-push.sh` together). Falls back to `main..@` if the function
  # is unavailable in the active jj.
  if changed=$(jj diff --name-only -r 'fork_point(main | @)..@' 2>/dev/null) \
