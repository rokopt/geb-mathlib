...
# (lib/scripts/check_axioms_inline.sh in that plugin's tree).
# Local modification: STANDARD_AXIOMS excludes Classical.choice
# (per geb-mathlib's constructive-only discipline).
# Re-vendor by re-copying from a newer lean4-skills release and
# re-applying the Classical.choice exclusion.
#
...
check_file() {
    ...
    if OUTPUT=$(lake env lean "$FILE" 2>&1); then
        ...
        while IFS= read -r line; do
            # Inline-list format:
            #   'declName' depends on axioms: [axiom1, axiom2]
            # Decl name is single-quoted; axioms are listed inline
            # in square brackets, comma-separated.
            if [[ "$line" =~ ^\'?([a-zA-Z0-9_.]+)\'?[[:space:]]+depends[[:space:]]+on[[:space:]]+axioms:[[:space:]]*(\[([^]]*)\])? ]]; then
                CURRENT_DECL="${BASH_REMATCH[1]}"
                local axiom_list="${BASH_REMATCH[3]:-}"
                if [[ "$VERBOSE" == "--verbose" ]]; then
                    echo -e "  ${BLUE}$CURRENT_DECL:${NC}"
                fi
                # Inline-list branch: parse axioms from the [list]
                # match group when present.
                if [[ -n "$axiom_list" ]]; then
                    local IFS_ORIG="$IFS"
                    IFS=',' read -ra _axioms <<< "$axiom_list"
                    IFS="$IFS_ORIG"
                    for axiom_raw in "${_axioms[@]}"; do
                        axiom="${axiom_raw#"${axiom_raw%%[![:space:]]*}"}"
            ...
            # Multi-line format: each axiom on its own line after the
            # depends-on header. Some Lean versions emit this shape
        ...
        if [[ "$HAS_REAL_ERROR" == false ]] && echo "$OUTPUT" | grep -q 'unknownIdentifier\|unknown identifier\|unknown constant'; then
            ...
            while IFS= read -r line; do
                # See parsing notes in the primary branch above.
                if [[ "$line" =~ ^\'?([a-zA-Z0-9_.]+)\'?[[:space:]]+depends[[:space:]]+on[[:space:]]+axioms:[[:space:]]*(\[([^]]*)\])? ]]; then
                    CURRENT_DECL="${BASH_REMATCH[1]}"
                    local axiom_list="${BASH_REMATCH[3]:-}"
                    PARSED_ANY=true
