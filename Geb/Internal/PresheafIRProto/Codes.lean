...
* `GebProto.iotaCode` / `GebProto.sigmaCode` / `GebProto.deltaCode` — the three
  code constructors.
...
* `GebProto.elObj_eq_of_hom` / `GebProto.elHom_eq_eqToHom_comp` — the source of
  ...
  that transport.
* `GebProto.interp_iotaCode`, `GebProto.interp_sigmaCode`,
  `GebProto.interp_deltaCode` — the interpretation's computation rules, one per
...
inductive family: the index is a base category, which `σ` replaces by a category
of elements. `Cat.{v, u}` is closed under that step because the category of
elements of a presheaf valued in `Type u` on a base in `Type u` is again in
`Type u`, with homs a subtype of the base's. The arity carrier universe is
...
With `hasBijectiveReindex_adjoinArityConst` this rules out `arityVaries` having a code
...
/-- The fused `δ` at an output-varying arity, its continuation the `ι` rule at
`decVariesElt`. -/
...
def codeAlgOn (𝔹 : Cat.{v, u}) :
  ...
  | Sum.inr (Sum.inr (Sum.inr (Sum.inl ⟨A, hA⟩))), c =>
      adjoinArity (c PUnit.unit) (A.pullback (c PUnit.unit).toPresheafPFunctorData)
        (A.isFunctorial_pullback hA (c PUnit.unit))
  | Sum.inr (Sum.inr (Sum.inr (Sum.inr ⟨A, hA⟩))), c => delta A hA D (c PUnit.unit)
...
set_option linter.checkUnivs false in
/-- The `ι` code over `𝔹` at an object `j₀`, the transcription of small
...
shapes. Its arity is over `ElObj D`, so a direction's label carries its own
decoding value and there is nothing left to sum over — the coproduct `delta`
takes is absorbed into the labelling, which is the collapse
`PSh(𝕀)/D ≃ PSh(el(D)ᵒᵖ)` seen from the arity side. -/
...
equality, which is small induction recursion's own `ι` semantics; see
`iotaDiscreteShapeEquiv`. -/
...
theorem interp_deltaPlainCode (𝔹 : Cat.{v, u})
    ...
    interp I D (deltaPlainCode I D 𝔹 A hA K hK) =
      ⟨𝔹, adjoinArity (cast (congrArg (fun 𝔻 : Cat.{v, u} ↦
          PresheafPFunctor.{u, u, max u v, u, u, v} (ElObj.{u, u, u} D) 𝔻)
