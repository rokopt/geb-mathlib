...
(Definitions 2.1–2.4 and Examples 2.5–2.6), which presents the theory
of [DybjerSetzer1999].
...
* `IRinterpObj`, `IRinterpMor` — the object-map and morphism-map
  ...
  `FreeCoprodCompDisc`.
* `univCode`, `univEndo`, `univEndoMor` — the code, and interpreted
  endofunctor, of the universe generated by an arbitrary family of
  starting types and closed under dependent sums and dependent
  products.
...
coproduct, over `UnivConstructor`, of the subcodes. Its interpretation
is the endofunctor whose initial algebra is the universe containing
the (lifted) starting types and closed under dependent sums and
dependent products. -/
...
/-- The object map of the endofunctor on the free coproduct completion
of `Type (max uK uT)` generated by `univCode`. -/
def univEndo :
  FreeCoprodCompDiscEndo.{max uK uT, max uK uT + 1} (Type (max uK uT)) :=
    IRinterpObj (Type (max uK uT)) (univCode K T)

/-- The morphism map of `univEndo`. -/
def univEndoMor :
