...
* {lit}`Direction` gives the variable directions of a free-polynomial shape.
* {lit}`Derived` specifies operations by terms in their arguments.
...
* {lit}`Presented` selects a definition by a structural direction in its block.
...
* {cite}`GambinoKock2013`, Theorem 4.5, for free-polynomial shapes and directions.
