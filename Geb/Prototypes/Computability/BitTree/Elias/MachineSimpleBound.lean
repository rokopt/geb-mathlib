...
theorem runFrom_tree_headBound (input : List (Fin 3)) (pos : Fin (input.length + 2))
    ...
    (t : ℕ) (ht : t ≤ 1) :
    HeadBound width (machine.runFrom (scanCfg input pos stTree pending 0 bs cs) t) := by
  apply runFrom_one_headBound _ _ width (step_tree input pos pending 0 bs cs b hin)
...
theorem runFrom_zeros_false_headBound (input : List (Fin 3)) (pos : Fin (input.length + 2))
    ...
    (t : ℕ) (ht : t ≤ 1) :
    HeadBound width (machine.runFrom (scanCfg input pos stZeros pending zeros bs cs) t) := by
  apply runFrom_one_headBound _ _ width (step_zeros input pos pending zeros bs cs false hin)
...
theorem runFrom_zeros_true_headBound (input : List (Fin 3)) (pos : Fin (input.length + 2))
    ...
    (t : ℕ) (ht : t ≤ 2) :
    HeadBound width (machine.runFrom (scanCfg input pos stZeros pending zeros [] [true]) t) := by
  ...
  rw [hs]
  intro r hr
  apply runFrom_one_headBound _ _ width hc
...
theorem runFrom_terminal_headBound {input : List (Fin 3)}
    ...
    (t : ℕ) (ht : t ≤ 1) : HeadBound width (machine.runFrom cfg t) := by
  apply runFrom_one_headBound cfg (machine.step cfg) width rfl hcfg ?_ t ht
  intro i
