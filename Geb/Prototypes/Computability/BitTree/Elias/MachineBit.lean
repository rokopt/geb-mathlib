...
* {lit}`runFrom_bit` proves the exact one-bit simulation and absence of output.
* {lit}`runFrom_bit_headBound` bounds every intermediate work-head position.
...
theorem step_terminal (input : List (Fin 3)) (pos : Fin (input.length + 2))
  ...
  rcases hq with rfl | rfl <;> refine Cfg.ext rfl rfl rfl ?_ rfl
  all_goals
    funext i
    change _ + (0 : ℤ) = _
...
theorem runFrom_zeros_positive (input : List (Fin 3)) (pos : Fin (input.length + 2))
    (pending zeros : ℕ) (hz : 0 < zeros)
    (hin : (scanCfg input pos stZeros pending zeros [] [true]).inputSymbol =
      some (boolEmb true)) :
    machine.runFrom (scanCfg input pos stZeros pending zeros [] [true]) 2 =
  ...
  exact runFrom_output_add _ _ _ 1 1 hread hcheck

/-- Every complete one-bit macro realizes the scalar scanner transition at its exact cost. -/
theorem runFrom_bit (input : List (Fin 3)) (pos : Fin (input.length + 2))
    ...
    machine.runFrom (modelCfg input pos s bs cs) (bitCost s.1 bs cs b) =
  ...
  | tree =>
    obtain ⟨rfl, rfl⟩ := hw
    have h := runFrom_output_one _ _ (step_tree input pos pending 0 [] [] b hin)
  ...
  | zeros zeros =>
    ...
    cases b with
    | false =>
      have h := runFrom_output_one _ _ (step_zeros input pos pending zeros [] [true] false hin)
    ...
    | true =>
      ...
      · subst zeros
        simpa only [Scanner.step, nextWords, bitCost, modelCfg, modeState, zeroCount,
          ↓reduceIte] using runFrom_empty_leaf input pos pending hs hin
      · simpa only [Scanner.step, nextWords, bitCost, modelCfg, modeState, zeroCount,
          hz, ↓reduceIte] using runFrom_zeros_positive input pos pending zeros (by omega) hin
  | size remaining v =>
    have h := runFrom_size input pos pending remaining bs cs b hs.2.1 (by rw [hw.1]; exact hs.2.2)
      hin
    by_cases he : remaining = 1 <;>
      simpa only [Scanner.step, nextWords, bitCost, modelCfg, modeState, zeroCount,
        he, ↓reduceIte, Nat.sub_self] using h
  | length remaining v =>
    have h := runFrom_length input pos pending remaining bs cs b hs.2.1 hw.1
      (by rw [hw.2]; exact hs.2.2) hin
    by_cases he : remaining = 1 <;>
      simpa only [Scanner.step, nextWords, bitCost, modelCfg, modeState, zeroCount,
        he, ↓reduceIte] using h
  | payload remaining =>
    have h := runFrom_payload input pos pending remaining bs cs b hs.1 hs.2 hw.2 hin
    by_cases he : remaining = 1 <;>
      simpa only [Scanner.step, nextWords, bitCost, modelCfg, modeState, zeroCount,
        he, ↓reduceIte] using h
  | done =>
    have h := runFrom_output_one _ _ (step_terminal input pos stDone pending 0 bs cs b
      (Or.inl rfl) hin)
      (outputSymbol_read _ stDone b rfl hin
        (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))))
    exact h
  | dead =>
    have h := runFrom_output_one _ _ (step_terminal input pos stDead pending 0 bs cs b
      (Or.inr rfl) hin)
...
theorem runFrom_bit_headBound (input : List (Fin 3)) (pos : Fin (input.length + 2))
    (s : Scanner.State) (bs cs : List Bool) (b : Bool)
    (hs : Scanner.Active s) (hw : Words s.1 bs cs)
    (hin : (modelCfg input pos s bs cs).inputSymbol = some (boolEmb b))
    ...
    HeadBound width (machine.runFrom (modelCfg input pos s bs cs) t) := by
  rcases s with ⟨m, pending⟩
  cases m with
  | tree => exact runFrom_tree_headBound input pos pending bs cs b width hp hb hc hin t ht
  | zeros zeros =>
    obtain ⟨rfl, rfl⟩ := hw
    cases b with
    | false =>
      exact runFrom_zeros_false_headBound input pos pending zeros [] [true] width
    ...
    | true =>
      by_cases he : zeros = 0
      · subst zeros
        exact runFrom_empty_leaf_headBound input pos pending width hs (by omega)
          (by simpa only [List.length_nil] using hb) hin t ht
      · have ht' : t ≤ 2 := by simpa only [bitCost, he, ↓reduceIte] using ht
        exact runFrom_zeros_true_headBound input pos pending zeros width (by omega)
          (by omega) (by change zeros + 1 ≤ width at hz; omega)
          (by simpa only [List.length_nil] using hb) hin t ht'
  | size remaining v =>
    exact runFrom_size_headBound input pos pending remaining bs cs b hs.2.1
      (by rw [hw.1]; exact hs.2.2) hin width hp hz hb hc t ht
  | length remaining v =>
    exact runFrom_length_headBound input pos pending remaining bs cs b hs.2.1 hw.1
      (by rw [hw.2]; exact hs.2.2) hin width hp hz hb hc t ht
  | payload remaining =>
    exact runFrom_payload_headBound input pos pending remaining bs cs b width hs.1 hs.2 hw.2
      (by omega) (by omega) (by omega) hin t ht
  | done =>
    exact runFrom_terminal_headBound _ stDone b width rfl (Or.inl rfl) hin
      (scanCfg_headBound input pos stDone pending 0 bs cs width
        (by omega) (by omega) (by omega) (by omega)) t ht
  | dead =>
    exact runFrom_terminal_headBound _ stDead b width rfl (Or.inr rfl) hin
