...
* {lit}`runFrom_payload` executes one raw payload bit.
* {lit}`runFrom_payload_headBound` bounds all intermediate head positions.
...
theorem runFrom_clear_scan (input : List (Fin 3)) (pos : Fin (input.length + 2))
    (pending : ℕ) (bs cs : List Bool) (hp : 0 < pending) :
    machine.runFrom (scanCfg input pos stClear pending 0 bs cs)
  ...
  have h := runFrom_clear_ready (scanCfg input pos stClear pending 0 bs cs)
...
theorem runFrom_payload (input : List (Fin 3)) (pos : Fin (input.length + 2))
    ...
    machine.runFrom (scanCfg input pos stPayloadBit pending 0 bs cs)
  ...
  have hread := runFrom_output_one _ _ (step_payload input pos pending 0 bs cs b hin)
    (outputSymbol_read _ stPayloadBit b rfl hin
      (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl rfl))))))
  have hdec := runFrom_decrement_true_scan input (moveInputPos pos 1) pending 0 bs cs
    (by omega)
  ...
  · have hseen : (Counter.decrement cs).any id = false := by
    ...
    rw [hseen] at hdec
    have hfirst := runFrom_output_add _ _ _ 1 (2 * cs.length + 3) hread hdec
    have hclear := runFrom_clear_scan input (moveInputPos pos 1) pending bs
      (Counter.decrement cs) hp
    rw [Counter.length_decrement] at hclear
    have hall := runFrom_output_add _ _ _ (1 + (2 * cs.length + 3))
  ...
  · have hseen : (Counter.decrement cs).any id = true := by
      ...
      | true => rfl
    rw [hseen] at hdec
    have hall := runFrom_output_add _ _ _ 1 (2 * cs.length + 3) hread hdec
...
theorem runFrom_payload_headBound (input : List (Fin 3)) (pos : Fin (input.length + 2))
    ...
    HeadBound width (machine.runFrom (scanCfg input pos stPayloadBit pending 0 bs cs) t) := by
  ...
  have hdecbound : ∀ r ≤ 2 * cs.length + 3, HeadBound width (machine.runFrom
  ...
  · have hseen : (Counter.decrement cs).any id = false := by
    ...
    have hdec := (runFrom_decrement_true_scan input (moveInputPos pos 1) pending 0 bs cs
      (by omega)).1
    rw [hseen] at hdec
    have hclear : ∀ r ≤ max bs.length cs.length + 3, HeadBound width (machine.runFrom
