...
* {lit}`runFrom_size` executes one bit of the binary width field.
* {lit}`runFrom_length` executes one bit of the payload-length field.
...
theorem runFrom_decrement_false_scan (input : List (Fin 3)) (pos : Fin (input.length + 2))
    (pending zeros : ℕ) (bs cs : List Bool) (hb : 0 < Counter.value bs) :
    machine.runFrom (scanCfg input pos (stDecrement false) pending zeros bs cs)
        (2 * bs.length + 3) =
    ...
    machine.outputString (scanCfg input pos (stDecrement false) pending zeros bs cs)
        (2 * bs.length + 3) = [] := by
  have h := runFrom_decrement (scanCfg input pos (stDecrement false) pending zeros bs cs)
    false bs hb
...
theorem runFrom_decrement_true_scan (input : List (Fin 3)) (pos : Fin (input.length + 2))
    (pending zeros : ℕ) (bs cs : List Bool) (hc : 0 < Counter.value cs) :
    machine.runFrom (scanCfg input pos (stDecrement true) pending zeros bs cs)
        (2 * cs.length + 3) =
    ...
    machine.outputString (scanCfg input pos (stDecrement true) pending zeros bs cs)
        (2 * cs.length + 3) = [] := by
  have h := runFrom_decrement (scanCfg input pos (stDecrement true) pending zeros bs cs)
    true cs hc
...
theorem runFrom_size (input : List (Fin 3)) (pos : Fin (input.length + 2))
    ...
    machine.runFrom (scanCfg input pos stSizeBit pending remaining bs cs)
  ...
  have hread := runFrom_output_one _ _ (hs hin)
    (outputSymbol_read _ stSizeBit b rfl hin (Or.inr (Or.inr (Or.inl rfl))))
  have hcheck := runFrom_output_one _ _
  ...
  have hfirst := runFrom_output_add _ _ _ 1 1 hread hcheck
  ...
  · subst remaining
    ...
    have hseen : (Counter.decrement (b :: bs)).any id = true := by
      ...
      | false =>
        ...
        omega
      | true => rfl
    have hdec := runFrom_decrement_false_scan input (moveInputPos pos 1) pending 0 (b :: bs) cs
      (by omega)
    rw [hseen] at hdec
    have h := runFrom_output_add _ _ _ 2 (2 * (b :: bs).length + 3) hfirst hdec
...
theorem runFrom_length (input : List (Fin 3)) (pos : Fin (input.length + 2))
    (pending remaining : ℕ) (bs cs : List Bool) (b : Bool)
    ...
    machine.runFrom (scanCfg input pos stLengthBit pending 0 bs cs)
  ...
  have hread := runFrom_output_one _ _ (step_length input pos pending 0 bs cs b hin)
    (outputSymbol_read _ stLengthBit b rfl hin (Or.inr (Or.inr (Or.inr (Or.inl rfl)))))
  have hdec := runFrom_decrement_false_scan input (moveInputPos pos 1) pending 0 bs (b :: cs)
    (by omega)
  have hfirst := runFrom_output_add _ _ _ 1 (2 * bs.length + 3) hread hdec
  by_cases he : remaining = 1
  · have hzero : (Counter.decrement bs).any id = false := by
    ...
    have hseen : (Counter.decrement (b :: cs)).any id = true := by
      ...
      | false =>
        ...
        omega
      | true => rfl
    have hlast := runFrom_decrement_true_scan input (moveInputPos pos 1) pending 0
      (Counter.decrement bs) (b :: cs) (by omega)
    rw [hseen] at hlast
    have h := runFrom_output_add _ _ _ (1 + (2 * bs.length + 3))
