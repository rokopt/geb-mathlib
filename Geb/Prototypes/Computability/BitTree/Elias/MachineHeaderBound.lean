...
theorem runFrom_size_headBound (input : List (Fin 3)) (pos : Fin (input.length + 2))
  ...
  have htwo : machine.runFrom (scanCfg input pos stSizeBit pending remaining bs cs) 2 =
      scanCfg input (moveInputPos pos 1)
        (if remaining - 1 = 0 then stDecrement false else stSizeBit)
        pending (remaining - 1) (b :: bs) cs := by
    rw [runFrom_succ_eq_step, hs hin, runFrom_succ_eq_step, runFrom_zero, step_sizeCheck]
  have hfirst : ∀ u ≤ 2, HeadBound width (machine.runFrom
    ...
    subst v
    rw [runFrom_zero, step_sizeCheck]
...
theorem runFrom_length_headBound (input : List (Fin 3)) (pos : Fin (input.length + 2))
    (pending remaining : ℕ) (bs cs : List Bool) (b : Bool)
  ...
  have hfirst : ∀ u ≤ 1 + (2 * bs.length + 3), HeadBound width (machine.runFrom
  ...
  · have hzero : (Counter.decrement bs).any id = false := by
    ...
    have hend : machine.runFrom (scanCfg input pos stLengthBit pending 0 bs cs)
        (1 + (2 * bs.length + 3)) =
      scanCfg input (moveInputPos pos 1) (stDecrement true) pending 0
        (Counter.decrement bs) (b :: cs) := by
      rw [Nat.add_comm 1, runFrom_succ_eq_step, step_length input pos pending 0 bs cs b hin,
        (runFrom_decrement_false_scan input (moveInputPos pos 1) pending 0 bs (b :: cs) hbpos).1,
        hzero]
      rfl
