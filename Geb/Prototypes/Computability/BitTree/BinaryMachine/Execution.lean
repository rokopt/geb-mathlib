...
* {lit}`runFrom_account` realizes every prefix account within its accumulated cost.
...
theorem headBound_start (input : List (Fin 4)) (width : ℕ) (hw : 1 ≤ width) :
    ∀ t ≤ 2, HeadBound width (machine.runFrom (machine.initCfg input) t) := by
  intro t ht i
...
theorem runFrom_account (w : List Bool) :
    ∀ t, t ≤ w.length →
      ...
      let now := machine.runFrom (machine.initCfg (w.map boolEmb)) cost
  ...
  · intro ht
    change Represents (w.length + 2).size
      (machine.runFrom (machine.initCfg (w.map boolEmb)) 2) stTree 1 0 ∧ _
    rw [runFrom_start]
  ...
  · intro t ih ht
    ...
    let cfg := machine.runFrom (machine.initCfg (w.map boolEmb)) (2 + runCost (w.take t))
    have hstep := runFrom_bit w t hlt cfg hp (account (w.take t)) (w.length + 2).size
      (account_valid _) hr hs.1 hs.2
    have hbound := runFrom_bit_head_bounds w t hlt cfg hp (account (w.take t))
    ...
    have he : 2 + runCost (w.take (t + 1)) =
      ...
      omega
    have hc : machine.runFrom (machine.initCfg (w.map boolEmb))
        (2 + runCost (w.take (t + 1))) =
        machine.runFrom cfg (macroCost (account (w.take t)) w[t]) := by
      rw [he, runFrom_add]
    dsimp only
    refine ⟨?_, ?_, ?_, ?_⟩
