...
* {lit}`runFrom_carry_invariants` maintains marker and mismatch invariants.
* {lit}`runFrom_increment` implements one increment in twice its bit-change count plus one step.
* {lit}`runFrom_increment_head_bounds` bounds every visited work-tape position.
...
theorem runFrom_carry_invariants {input : List (Fin 4)}
  ...
  · intro t ih ht
    ...
    obtain ⟨hstate, hpos0, hpos1, _⟩ :=
      runFrom_carry_prefix cfg leaf p k hq hp0 hp1 hones t (by omega)
...
theorem runFrom_carry_bits_invariants {input : List (Fin 4)}
  ...
  apply runFrom_carry_invariants cfg leaf 0 (Counter.flips bs - 1) width
...
theorem runFrom_increment {input : List (Fin 4)}
  ...
  let a' := if leaf then a else a + 1
  let b' := if leaf then b + 1 else b
  let after := machine.runFrom cfg f
  ...
  obtain ⟨hstate, hpos0, hpos1, hin, hselected, hother, hout⟩ :=
    runFrom_carry_bits cfg leaf n.bits h.state h.forkPos h.leafPos hbits
  ...
  have hm : cfg.workTapePos 2 = (mismatchCount width (digits cfg 0) (digits cfg 1) : ℤ) := by
    ...
    exact h.mismatchPos
  obtain ⟨horigin, hmarker, hmismatch⟩ := runFrom_carry_bits_invariants cfg leaf n.bits
  ...
  obtain ⟨hc, ho'⟩ := runFrom_return_done after ho f
  rw [he] at hc ho'
  have hfinal : machine.runFrom cfg (2 * f + 1) = returnedCfg after := by
...
theorem runFrom_increment_head_bounds {input : List (Fin 4)}
    (cfg : Cfg 3 (Fin 4) (Fin 10) input) (leaf : Bool) (width a b : ℕ)
    (h : Represents width cfg (if leaf then stCarryLeaf else stCarryFork) a b)
    (hnext : ((if leaf then b else a) + 1).size ≤ width)
    (t : ℕ) (ht : t ≤ 2 * Counter.flips (if leaf then b else a).bits + 1) (i : Fin 3) :
    0 ≤ (machine.runFrom cfg t).workTapePos i ∧
      (machine.runFrom cfg t).workTapePos i ≤ width := by
  let n := if leaf then b else a
  let f := Counter.flips n.bits
  have hw : f ≤ width := (Counter.flips_bits_le_succ_size n).trans hnext
  have hbits : ∀ j, digits cfg (if leaf then 1 else 0) j = n.bits[j]?.getD false := by
  ...
  by_cases htf : t ≤ f
  · obtain ⟨hp0, hp1⟩ :=
      runFrom_carry_bits_pos cfg leaf n.bits h.state h.forkPos h.leafPos hbits t htf
    have hm' := (runFrom_carry_bits_invariants cfg leaf n.bits width h.state
      h.forkPos h.leafPos hbits hw hm t htf).2.2
    have hcount := mismatchCount_le width (digits (machine.runFrom cfg t) 0)
      (digits (machine.runFrom cfg t) 1)
    fin_cases i
    · change 0 ≤ (machine.runFrom cfg t).workTapePos 0 ∧
        (machine.runFrom cfg t).workTapePos 0 ≤ width
      rw [hp0]
      omega
    · change 0 ≤ (machine.runFrom cfg t).workTapePos 1 ∧
        (machine.runFrom cfg t).workTapePos 1 ≤ width
      rw [hp1]
      omega
    · change 0 ≤ (machine.runFrom cfg t).workTapePos 2 ∧
        (machine.runFrom cfg t).workTapePos 2 ≤ width
      rw [hm']
      omega
  · let after := machine.runFrom cfg f
    obtain ⟨hq, hp0, hp1, _⟩ :=
      runFrom_carry_bits cfg leaf n.bits h.state h.forkPos h.leafPos hbits
    obtain ⟨horigin, _, hm'⟩ := runFrom_carry_bits_invariants cfg leaf n.bits width
    ...
    have hb := runFrom_return_head_bounds after ho f width hw
