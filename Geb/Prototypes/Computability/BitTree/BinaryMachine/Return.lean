...
* {lit}`runFrom_return`: exact configurations during the return.
* {lit}`runFrom_return_done`: the complete return, including its final state change.
...
theorem tr_return_pos {input : List (Fin 4)}
    ...
    machine.tr stReturn (returnCfg cfg (p + 1)).inputSymbol
      ...
      { inputTape := 0, workTapes := fun i ↦ (none, if i = 2 then 0 else -1),
        output := none, state := some stReturn } := by
...
theorem runFrom_return {input : List (Fin 4)}
  ...
  · intro t ih p
    ...
    constructor
    · rw [runFrom_succ_eq_step', hc, step_return_pos cfg p ho]
...
theorem tr_return_zero {input : List (Fin 4)}
    ...
    machine.tr stReturn (returnCfg cfg 0).inputSymbol (returnCfg cfg 0).workTapeSymbols =
      { inputTape := 0, workTapes := fun _ ↦ (none, 0), output := none,
        state := some (if cfg.workTapes 2 (cfg.workTapePos 2) == some 0
...
theorem runFrom_return_done {input : List (Fin 4)}
    (cfg : Cfg 3 (Fin 4) (Fin 10) input)
    (ho : ∀ z, origin (cfg.workTapes 0 z) = decide (z = 0)) (p : ℕ) :
    machine.runFrom (returnCfg cfg p) (p + 1) = returnedCfg cfg ∧
      machine.outputString (returnCfg cfg p) (p + 1) = [] := by
  obtain ⟨hc, hout⟩ := runFrom_return cfg ho p 0
  simp only [Nat.zero_add] at hc hout
  constructor
  · rw [runFrom_succ_eq_step', hc, step_return_zero cfg ho]
...
theorem runFrom_return_head_bounds {input : List (Fin 4)}
    (cfg : Cfg 3 (Fin 4) (Fin 10) input)
    (ho : ∀ z, origin (cfg.workTapes 0 z) = decide (z = 0))
    (p width : ℕ) (hp : p ≤ width)
    (hm0 : 0 ≤ cfg.workTapePos 2) (hmw : cfg.workTapePos 2 ≤ width)
    (t : ℕ) (ht : t ≤ p + 1) (i : Fin 3) :
    0 ≤ (machine.runFrom (returnCfg cfg p) t).workTapePos i ∧
      (machine.runFrom (returnCfg cfg p) t).workTapePos i ≤ width := by
  by_cases htp : t ≤ p
  · have hc := (runFrom_return cfg ho t (p - t)).1
    rw [show p - t + t = p by omega] at hc
    rw [hc]
    fin_cases i <;> simp only [returnCfg] <;> omega
  · have he : t = p + 1 := by omega
    subst t
    rw [(runFrom_return_done cfg ho p).1]
