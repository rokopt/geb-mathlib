...
* {lit}`runFrom_bit` implements one bit without emitting output.
* {lit}`runFrom_bit_head_bounds` bounds the heads throughout that segment.
...
theorem runFrom_bit_aux {input : List (Fin 4)}
    ...
    Represents width (machine.runFrom cfg cost) (modeState next.mode) next.forks next.leaves ∧
      (machine.runFrom cfg cost).inputPos = moveInputPos cfg.inputPos 1 ∧
  ...
  have hc : machine.runFrom cfg 1 = after := by
  ...
  case tree.true =>
    ...
    obtain ⟨hr', hp', ho'⟩ := runFrom_increment after false width a c hr hnext
    simp only [Bool.false_eq_true, ↓reduceIte] at hr' hp' ho'
    have hne : a + 1 ≠ c := by
      change c ≤ a ∧ 0 < a ∧ c < a at hv
      omega
    have hc' : machine.runFrom cfg (macroCost ⟨.tree, a, c⟩ true) =
        machine.runFrom after (2 * Counter.flips a.bits + 1) := by
      change machine.runFrom cfg (1 + (2 * Counter.flips a.bits + 1)) = _
      rw [runFrom_add, hc]
    refine ⟨?_, ?_, ?_, ?_⟩
    · rw [hc']
    ...
    · apply headBound_add cfg width 1 (2 * Counter.flips a.bits + 1) hh
      intro u hu
      rw [hc]
      exact fun i ↦ runFrom_increment_head_bounds after false width a c hr hnext u hu i
  case string.false =>
    have hnext : (c + 1).size ≤ width := by simpa [accountStep] using hb
    obtain ⟨hr', hp', ho'⟩ := runFrom_increment after true width a c hr hnext
    ...
    have hmode : modeState (accountStep ⟨.string, a, c⟩ false).mode =
      ...
      · have he' : a - c ≠ 1 := by omega
        ...
        rfl
    have hc' : machine.runFrom cfg (macroCost ⟨.string, a, c⟩ false) =
        machine.runFrom after (2 * Counter.flips c.bits + 1) := by
      change machine.runFrom cfg (1 + (2 * Counter.flips c.bits + 1)) = _
      rw [runFrom_add, hc]
    refine ⟨?_, ?_, ?_, ?_⟩
    · rw [hc', hmode]
      exact hr'
    ...
    · apply headBound_add cfg width 1 (2 * Counter.flips c.bits + 1) hh
      intro u hu
      rw [hc]
      exact fun i ↦ runFrom_increment_head_bounds after true width a c hr hnext u hu i
  all_goals
    change Represents width (machine.runFrom cfg 1) _ a c ∧
      (machine.runFrom cfg 1).inputPos = moveInputPos cfg.inputPos 1 ∧
      machine.outputString cfg 1 = [] ∧ ∀ u ≤ 1, HeadBound width (machine.runFrom cfg u)
    refine ⟨?_, ?_, ho, hh⟩
    · rw [hc]
      exact hr
    · rw [hc]
...
theorem runFrom_bit (w : List Bool) (t : ℕ) (ht : t < w.length)
    ...
    Represents width (machine.runFrom cfg cost) (modeState next.mode) next.forks next.leaves ∧
      (machine.runFrom cfg cost).inputPos.val = t + 2 ∧ machine.outputString cfg cost = [] := by
  obtain ⟨hr, hp', ho, _⟩ := runFrom_bit_aux cfg s w[t] width hv h
...
theorem runFrom_bit_head_bounds (w : List Bool) (t : ℕ) (ht : t < w.length)
    (cfg : Cfg 3 (Fin 4) (Fin 10) (w.map boolEmb)) (hp : cfg.inputPos.val = t + 1)
    (s : Account) (width : ℕ) (hv : AccountValid s)
    (h : Represents width cfg (modeState s.mode) s.forks s.leaves)
    (ha : (accountStep s w[t]).forks.size ≤ width)
    (hb : (accountStep s w[t]).leaves.size ≤ width) :
    ∀ u ≤ macroCost s w[t], HeadBound width (machine.runFrom cfg u) :=
  (runFrom_bit_aux cfg s w[t] width hv h (inputSymbol_at w t ht cfg hp) ha hb).2.2.2
