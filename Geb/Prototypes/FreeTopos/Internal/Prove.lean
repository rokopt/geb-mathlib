...
given, each premise by normalization. The prover is not trusted: a derivation it computes is
checked.
...
* {lit}`byNorm`, {lit}`byNatInd`, {lit}`byListInd` — the proofs of an equation.
...
/-- A rule the normalizer applies at a term's root: one of the language's equations, or the
theorem of an index at objects, whose instance is found by matching its left side. -/
inductive NormRule where
  ...
  | delta (k : ℕ)
  /-- The theorem of an index at objects, from its left side to its right. -/
  | thm (j : ℕ) (θ : List Tree)
...
def rootRewrite (rs : List NormRule) (Γ : List Tree) (t : Term) : Option (Term × Deriv) :=
  rs.foldl (fun acc r ↦ acc.orElse fun _ ↦ match r with
    | .rule l => (rootStep G E n Γ l t).map fun t' ↦ (t', RoseTree.node l [])
    | .delta k => match t.label with
      | .defn k' _ => if k = k' then
          (rootStep G E n Γ .delta t).map fun t' ↦ (t', RoseTree.node .delta [])
        else none
      | _ => none
    | .thm j θ => do
      let a ← E[j]?
      let σ ← matchTerm (Term.osubst θ a.lhs) t (List.replicate a.ctx.length none)
      let σ ← σ.mapM id
      let l := Rule.thm j θ σ false
      (rootStep G E n Γ l t).map fun t' ↦ (t', RoseTree.node l [])) none

/-- The normal form of a term in a context under rules, innermost first, within a depth of
{lit}`fuel`, with its rewriting derivation, marked when it rewrites. -/
def normalize (rs : List NormRule) (fuel : ℕ) : List Tree → Term → Option (Term × Deriv × Bool) :=
  fuel.rec (fun _ t ↦ some (t, dRefl)) fun _ rec Γ t ↦ do
    let Γs ← childCtxs G n t.label t.children Γ
    let cs ← (Γs.zip t.children).mapM fun (Δ, u) ↦ rec Δ u
    let t₁ := RoseTree.node t.label (cs.map Prod.fst)
    let d₁ : Deriv × Bool := if cs.any (·.2.2) then
        (RoseTree.node .cong (cs.map (·.2.1)), true)
      else dRefl
    match rootRewrite G E n rs Γ t₁ with
    | some (t₂, dr) => do
      let (t₃, d₃) ← rec Γ t₂
...
def byNorm (rs : List NormRule) (fuel : ℕ) (Γ : List Tree) (t u : Term) : Option Deriv := do
  let (v, d₁, _) ← normalize G E n rs fuel Γ t
  let (v', d₂, _) ← normalize G E n rs fuel Γ u
...
def byNatInd (kz ks : ℕ) (s : Term) (rs : List NormRule) (fuel : ℕ) (Γ : List Tree)
  ...
  | _ :: Γ' => do
    let p₀ ← byNorm G E n rs fuel Γ' (Term.subst t (instVar (Term.arr kz [] Term.star)))
      (Term.subst u (instVar (Term.arr kz [] Term.star)))
    let p₁ ← byNorm G E n rs fuel Γ (natSuccAt ks t) (Term.subst s (atVar0 t))
    let p₂ ← byNorm G E n rs fuel Γ (natSuccAt ks u) (Term.subst s (atVar0 u))
...
def byListInd (kn kc : ℕ) (s : Term) (rs : List NormRule) (fuel : ℕ) (Γ : List Tree)
  ...
  | c :: Γ' => do
    let a ← listPart c
    let p₀ ← byNorm G E n rs fuel Γ' (Term.subst t (instVar (Term.arr kn [a] Term.star)))
      (Term.subst u (instVar (Term.arr kn [a] Term.star)))
    let p₁ ← byNorm G E n rs fuel (c :: a :: Γ') (listConsAt kc a t)
      (Term.subst s (atVar0 (weakenElem t)))
    let p₂ ← byNorm G E n rs fuel (c :: a :: Γ') (listConsAt kc a u)
