...
conclusion; its result is a function of the global environment, the context and the
hypotheses, as the denotation's is. Its soundness is proved against the denotation
...
/-- The checker's result at a certificate: the conclusion, as a function of the global
environment, the context and the hypotheses, or nothing when the certificate does not check. -/
abbrev Chk : Type := List Glob → Ctx → List Eqn → Option Eqn

/-- One rule of the checker, by the label of a certificate's node: its children are the
premises' certificates, with their results, and the terms and types the rule names. -/
def checkStep (l : ℕ) (cs : List (Tree × Chk)) : Chk := fun G Γ H ↦
  match l, cs with
  ...
  | 2, [(_, p)] => (p G Γ H).map fun q ↦ ⟨q.ty, q.rhs, q.lhs⟩
  | 3, [(_, p), (_, p')] => (p G Γ H).bind fun q ↦ (p' G Γ H).bind fun q' ↦
  ...
  | 4, [(_, p), (_, p')] => (p G Γ H).bind fun q ↦ (p' G Γ H).bind fun q' ↦
    (arrowParts q.ty).bind fun AB ↦
      if q'.ty = AB.1 then some ⟨AB.2, mk 10 [q.lhs, q'.lhs], mk 10 [q.rhs, q'.rhs]⟩ else none
  | 5, [(A, _), (_, p)] =>
    if Ty.IsTy A then
      (p G (A :: Γ) (H.map (Eqn.wk 1))).map fun q ↦
        ⟨tArrow A q.ty, mk 9 [A, q.lhs], mk 9 [A, q.rhs]⟩
    else none
  | 6, [(_, p), (_, p')] => (p G Γ H).bind fun q ↦ (p' G Γ H).map fun q' ↦
    ⟨tProd q.ty q'.ty, mk 12 [q.lhs, q'.lhs], mk 12 [q.rhs, q'.rhs]⟩
  | 7, [(_, p)] => (p G Γ H).bind fun q ↦ (prodParts q.ty).map fun AB ↦
    ⟨AB.1, mk 13 [q.lhs], mk 13 [q.rhs]⟩
  | 8, [(_, p)] => (p G Γ H).bind fun q ↦ (prodParts q.ty).map fun AB ↦
    ⟨AB.2, mk 14 [q.lhs], mk 14 [q.rhs]⟩
  | 9, [(_, p), (_, p')] => (p G Γ H).bind fun q ↦ (p' G Γ H).bind fun q' ↦
    if q'.ty = tList q.ty then some ⟨q'.ty, mk 20 [q.lhs, q'.lhs], mk 20 [q.rhs, q'.rhs]⟩ else none
  | 10, [(_, p), (_, p'), (_, p'')] =>
    (p G Γ H).bind fun q ↦ (p' G Γ H).bind fun q' ↦ (p'' G Γ H).bind fun q'' ↦
  ...
  | 18, [(_, p)] =>
    match Γ with
    | _ :: Γ' => (p G Γ' []).map (Eqn.wk 1)
    | [] => none
  | 19, [(_, p), (_, p')] => (p G Γ H).bind fun h ↦ p' G Γ (h :: H)
  | 20, [(u, _), (_, p)] => (typeOf G Γ u).bind fun A ↦
    (p G (A :: Γ) (H.map (Eqn.wk 1))).map fun q ↦ ⟨q.ty, subst u q.lhs, subst u q.rhs⟩
  ...
  | 23, [(s, _), (t, _), (_, p0), (_, p1)] =>
    ...
    | L :: Γ' => (listParts L).bind fun A ↦ (typeOf G Γ s).bind fun B ↦
      ...
      if Ty.IsTy A ∧ typeOf G Γ t = some B ∧ H0.map (Eqn.wk 1) = H ∧ (∀ h ∈ H0, h.Typed G Γ') ∧
          p0 G Γ' H0 = some ⟨B, subst (mk 19 [A]) s, subst (mk 19 [A]) t⟩ ∧
          p1 G (L :: A :: Γ') (⟨B, wkAt 1 1 s, wkAt 1 1 t⟩ :: H0.map (Eqn.wk 2)) =
...
/-- The checker: the conclusion of a certificate, in a global environment, a context and a list
of hypotheses, or nothing when the certificate does not check. -/
...
/-- Every conclusion the checker computes is valid. -/
theorem check_sound : ∀ (c : Tree) (G : List Glob) (Γ : Ctx) (H : List Eqn) (q : Eqn),
    check c G Γ H = some q → Valid G Γ H q := by
  refine RoseTree.ind fun l cs ih G Γ H q h ↦ ?_
  ...
  case h_3 cp p heq =>
    ...
    exact valid_symm (ih cp hc G Γ H q' hq')
  case h_4 cp p cp' p' heq =>
    ...
    · rename_i hcond
      cases h
      exact valid_trans (ih cp hc G Γ H q1 h1) (ih cp' hc' G Γ H q2 h2) hcond.1 hcond.2
    · cases h
  case h_5 cp p cp' p' heq =>
    ...
    · rename_i hx
      cases h
      exact valid_app (ih cp hc G Γ H q1 h1) (ih cp' hc' G Γ H q2 h2) hAB hx
    · cases h
  case h_6 A sA cp p heq =>
    ...
    · rename_i hA
      obtain ⟨q', hq', rfl⟩ := Option.map_eq_some_iff.mp h
      exact valid_lam hA (ih cp hc G (A :: Γ) (H.map (Eqn.wk 1)) q' hq')
    · cases h
  case h_7 cp p cp' p' heq =>
    ...
    exact valid_pair (ih cp hc G Γ H q1 h1) (ih cp' hc' G Γ H q2 h2)
  case h_8 cp p heq =>
    obtain ⟨hc, rfl⟩ := mem_of_map_para (c := cp) (s := p) heq (by simp)
    obtain ⟨q1, h1, h⟩ := Option.bind_eq_some_iff.mp h
    obtain ⟨⟨A, B⟩, hAB, rfl⟩ := Option.map_eq_some_iff.mp h
    exact valid_fst (ih cp hc G Γ H q1 h1) hAB
  case h_9 cp p heq =>
    obtain ⟨hc, rfl⟩ := mem_of_map_para (c := cp) (s := p) heq (by simp)
    obtain ⟨q1, h1, h⟩ := Option.bind_eq_some_iff.mp h
    obtain ⟨⟨A, B⟩, hAB, rfl⟩ := Option.map_eq_some_iff.mp h
    exact valid_snd (ih cp hc G Γ H q1 h1) hAB
  case h_10 cp p cp' p' heq =>
    ...
    · rename_i hl
      cases h
      exact valid_cons (ih cp hc G Γ H q1 h1) (ih cp' hc' G Γ H q2 h2) hl
    · cases h
  case h_11 cp p cp' p' cp'' p'' heq =>
    ...
    · rename_i hcond
      cases h
      exact valid_if (ih cp hc G Γ H q1 h1) (ih cp' hc' G Γ H q2 h2) (ih cp'' hc'' G Γ H q3 h3)
        hcond.1 hcond.2
    · cases h
  ...
  case h_19 cp p heq =>
    ...
    · rename_i A Γ'
      obtain ⟨q', hq', rfl⟩ := Option.map_eq_some_iff.mp h
      exact valid_weaken (ih cp hc G Γ' [] q' hq')
    · cases h
  case h_20 cp p cp' p' heq =>
    obtain ⟨hc, rfl⟩ := mem_of_map_para (c := cp) (s := p) heq (by simp)
    obtain ⟨hc', rfl⟩ := mem_of_map_para (c := cp') (s := p') heq (by simp)
    obtain ⟨hq, hq1, h⟩ := Option.bind_eq_some_iff.mp h
    exact valid_cut (ih cp hc G Γ H hq hq1) (ih cp' hc' G Γ (hq :: H) q h)
  case h_21 u su cp p heq =>
    obtain ⟨hc, rfl⟩ := mem_of_map_para (c := cp) (s := p) heq (by simp)
    obtain ⟨A, hA, h⟩ := Option.bind_eq_some_iff.mp h
    obtain ⟨q', hq', rfl⟩ := Option.map_eq_some_iff.mp h
    exact valid_inst hA (ih cp hc G (A :: Γ) (H.map (Eqn.wk 1)) q' hq')
  ...
  case h_24 s ss t st cp0 p0 cp1 p1 heq =>
    ...
    · rename_i L Γ'
      ...
      · rename_i hcond
        ...
        exact valid_indList hA hB ht hH hT (ih cp0 hc0 _ _ _ _ hp0) (ih cp1 hc1 _ _ _ _ hp1)
      · cases h
    · cases h
  case h_25 => cases h
